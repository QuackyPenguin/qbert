...
from characters.player import JUMP_DURATION
...
class Level:
    ...
    def next_color(self, number):
        ...
        if new_color != old_color and new_color == self.colors[len(self.colors) - 1]:
            variables.score += 25
        elif new_color != old_color and new_color == self.colors[1]:
            variables.score += 15
        ...
        if flag:
            ...
            if self.level == 1 or self.level == 3 or (self.level == 0 and (self.round == 0 or self.round == 2)):
                if self.round == 1 or self.level == 0:
                    self.colors = [COLOR_BLUE, COLOR_YELLOW]
                elif self.round == 2:
                ...
                if self.round == 1 or self.level == 0:
                    self.colors = [COLOR_ORANGE, COLOR_BLUE, COLOR_YELLOW]
                elif self.round == 2:
    ...
    def init_next_level(self):
        variables.speed = max(0, variables.speed - 1)
    ...
    def next_enemy_appearance(self):
        a = max(5 - self.level // 2, 3)
        b = max(9 - self.level // 2 - self.round // 2, 5)
    ...
    def generate_enemy(self) -> Enemy:
        ...
        if self.level == 1 and (self.round == 1 or self.round == 4):
            red_ball_odds = red_ball_odds + 1000 * (self.round % 2)
            red_ball_odds -= number_of_red_balls * 1000 / 4.5
        ...
        elif (self.level == 1 and self.round == 3) or (self.level == 2 and self.round == 3) or (
            ...
            odds_array.append(slick_odds)
            case_array.append(SLICK)
        elif self.level == 1 and self.round == 2:
