...
class DuelingDQNAgent:
    def __init__(self, game: QbertGameAI, input_dims, n_actions, batch_size = 32, epsilon = 1.0, gamma = 0.9, 
        ...
        self.q_eval = DuelingDeepQNetwork(input_dims, n_actions, lr=self.lr, filename='duelingq_eval')
        self.q_next = DuelingDeepQNetwork(input_dims, n_actions, lr=self.lr, filename='duelingq_next')
    ...
    def get_action(self, state, evaluate=False):
        ...
        if np.random.random() < self.epsilon[self.index] and not evaluate:
            ...
            _, advantage = self.q_eval(state_tensor.unsqueeze(0))
